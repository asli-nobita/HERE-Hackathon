...
const requestIsolineShape = (options) => {
  ...
  return new Promise((resolve, reject) => {
    router.calculateIsoline(
      params,
      (res) => {
        const shape = res.response.isoline[0].component[0].shape.map((z) =>
          z.split(",")
        );
        resolve(shape);
      },
...
const autocompleteGeocodeUrl = (query) =>
  `https://autocomplete.geocoder.ls.hereapi.com/6.2/suggest.json?apiKey=nlyxv5Ld270xr0N8HARuYHa9sccBN04ZiGXs0eAHEf0
 &resultType=areas
 &query=${query}`;
